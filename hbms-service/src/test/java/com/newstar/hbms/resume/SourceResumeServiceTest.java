...
public class SourceResumeServiceTest extends TestCase {
  ...
  public void testEsMapping() throws IOException {
    Client client = TransportClient.builder().build()
        .addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("localhost"), 9300));
    PutMappingRequest request = Requests.putMappingRequest("hbms").type("resume").source(XContentFactory.jsonBuilder()
    .startObject()
      .startObject("resume")
        .startObject("_all")
  ...
  public void testInsertEs() throws IOException {
    List<SourceResume> sourceResumes = sourceResumeService.findByIds(new Long[]{5L});
    SourceResume sourceResume = sourceResumes.get(0);
    Client client = TransportClient.builder().build()
        .addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("localhost"), 9300));
    IndexResponse response = client.prepareIndex("hbms", "resume", String.valueOf(sourceResume.getId()))
